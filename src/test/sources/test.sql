--insert into employees ( manager_id, user_id, accesses, salary) values(2,3,123,1000);
...
INSERT INTO accounts (user_id, `name`) VALUE (1, 'Gregory');
INSERT INTO accounts (user_id, `name`) VALUE (2, 'Dmitry');
INSERT INTO accounts (user_id, `name`) VALUE (3, 'Oleg');
INSERT INTO accounts (user_id, `name`) VALUE (4, 'Leha');

delete employees, users, accounts
from employees e, users u, accounts a
where e.user_id = u.user_id and a.user_id = u.user_id and u.user_id = 1;

INSERT INTO accounts (user_id, `name`) VALUE (4, 'Lol');